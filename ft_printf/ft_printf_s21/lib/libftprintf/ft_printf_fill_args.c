...
{
	return (
		(!(fmt->length[0] == 'l' && fmt->length[1] == 'l') && fmt->type == 's' && ft_contains("lLjzt", *fmt->length) && *fmt->value != 0 && !is_null_pointer(fmt))
		|| (!(fmt->length[0] == 'l' && fmt->length[1] == 'l') && fmt->type == 'c' && *fmt->value < 0 && (ft_contains("tlLjz", *fmt->length)))
		);
}
